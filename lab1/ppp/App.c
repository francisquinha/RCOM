...
volatile bool* show_display;

//bool image_loaded = NO; //check with image bytes length nstead
volatile unsigned int* image_already_bytes;//num of image's bytes sent or received
char* image_bytes;
volatile unsigned int* image_bytes_length;
...
int sendImage() {
	...
	//send
	int ret = 0;
	ret = sendFile(app.fd, image_name_length, image_name, *image_bytes_length, image_bytes, image_already_bytes);
...
int main(int argc, char** argv)
{
	...
	volatile bool* show_display = (volatile bool*)malloc(sizeof(bool));
	volatile unsigned int* image_already_bytes = (volatile unsigned int*)malloc(sizeof(int));
	volatile unsigned int* image_bytes_length = (volatile unsigned int*)malloc(sizeof(int));
	...
	free(show_display);
	free(image_already_bytes);
	free(image_bytes_length);
