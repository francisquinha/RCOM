...
void set_basic_definitions(int timeout_in_seconds, int number_of_tries_when_failing, char* port, int baudrate,int iframedatasize)
{
	...
	link_layer.Iframe_numdatabytes_withoutstuffing = iframedatasize;
...
void write_I(int SorR,int fd, char* data, int* data_index)
{
	char msg[4];
	...
	/*TODO: LEFT WHILE WORKING ON THIS*/
	link_layer_data.Iframe_numdatabytes_withoutstuffing;
...
int update_received_data(char* data_buffer, int data_buffer_length, char* newdata_buffer, int newdata_buffer_length)
{
	...
	{
		if ((buffer = (char*)malloc(newdata_buffer_length)) == NULL)
		{
	...
	{
		buffer = (char*)realloc(buffer, data_buffer_length + newdata_buffer_length);
...
int llread(int fd,char* buffer)
{
	...
	char auxReceiveDataBuf[LLREAD_AUXDATABUFFER_SIZE];//aux buffer used to store data before
	...
	while (TRUE)//RECEIVE)
	{
		...
		for (; i < res; ++i)
		{
			update_state_machine(APP_STATUS_RECEIVER, APP_STATUS_TRANSMITTER, MESSAGE_RR, auxReadBuf[i], &state);
			//if (state == STATE_MACHINE_START;) lastStart = i;

			//receive data to auxReceiveDataBuf
			if ((state == STATE_MACHINE_BCC_RCV || state == STATE_MACHINE_ESCAPE) && received_C_type == MESSAGE_I)
			{
				auxReceiveDataBuf_length[auxReceiveDataBuf_length] = auxReadBuf[i];
				//memmov(auxReceiveDataBuf+, auxReadBuf + lastStart, i - lastStart);//-1 to discard end flag
				++auxReceiveDataBuf_length;
			}

			if (state == STATE_MACHINE_STOP){

				if (received_C_type != MESSAGE_I && auxReceiveDataBuf_length!=0)//BE EXTRA CAREFULL
				{
					/*clear buf*/memset(auxReceiveDataBuf, 0, LLREAD_AUXDATABUFFER_SIZE);
					auxReceiveDataBuf_length = 0;
				}

				if (received_C_type == MESSAGE_I)
				{
					int ns=0;
					if (TRUE)//validateData(int* ns))//deve atualizar o NR de acordo com o que encontrar
					{
						...
						update_received_data(buf,bufferSize,auxReceiveDataBuf,auxReceiveDataBuf_length);
						/*clear buf*/memset(auxReceiveDataBuf, 0, LLREAD_AUXDATABUFFER_SIZE);
						auxReceiveDataBuf_length=0;
						write_UorS(APP_STATUS_RECEIVER, MESSAGE_RR, NR, fd);
					}
					else write_UorS(APP_STATUS_RECEIVER, MESSAGE_REJ, 0, fd);
				}
				...
				else if (received_C_type == MESSAGE_SET)//SEND UA
					write_UorS(APP_STATUS_TRANSMITTER, MESSAGE_UA, 0, fd);
